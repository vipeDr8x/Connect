...
type FlowPhase = 'selection' | 'simple-chat' | 'profile-chat' | 'history' | 'user2-reply';
...
export default function MainRouterScreen() {
  ...
  const [activeChatMode, setActiveChatMode] = useState<'simple' | 'profile'>('simple');
  ...
  const isUserBlind = userDisabilities.includes('blind') || userDisabilities.length === 0;
  const isUserDeaf = userDisabilities.includes('deaf');
  const isUserMute =
    userDisabilities.includes('mute') || userDisabilities.includes('non-verbal');
  ...
  useEffect(() => {
    ...
    if (phase === 'selection') {
      ...
      if (isSevereIntersectionWIP) {
        speakScreenLayout('Профилен диалог. Функцията се разработва. Натиснете центъра за връщане назад.');
      } else if (isUserBlind) {
        speakScreenLayout(
        ...
        speakScreenLayout('Профилен диалог. Натиснете центъра на екрана за изпращане на данни.');
      }
  ...
  // ==========================================
  const commitMessage = (sender: 'user1' | 'user2', text: string, isVoice: boolean) => {
  ...
  if (phase === 'simple-chat') {
    return (
      <View style={styles.container}>
        ...
        <View style={[styles.half, { backgroundColor: BG, paddingTop: NOTCH_OFFSET, paddingHorizontal: 15 }]}>
          <Text style={styles.halfLabel}>Въвеждане на Текст (Потребител 1)</Text>
          <TextInput
        ...
        {/* BOTTOM HALF */}
        <View style={[styles.half, { backgroundColor: BG, paddingTop: 20, paddingHorizontal: 15 }]}>
          <Text style={styles.halfLabel}>Въвеждане на Глас (Потребител 1)</Text>
          <TouchableOpacity
            style={[styles.halfButton, { backgroundColor: isRecording ? '#E63946' : BTN }]}
          ...
          <TouchableOpacity style={styles.backLink} onPress={() => setPhase('selection')}>
  ...
  if (phase === 'profile-chat') {
    if (isSevereIntersectionWIP) {
      return (
        <View style={[styles.container, { justifyContent: 'center', alignItems: 'center', padding: 20 }]}>
          <Text style={styles.wipText}>Функцията се разработва за този профил.</Text>
          <TouchableOpacity
        ...
        <View style={styles.container}>
          ...
          >
            <Text style={styles.halfLabel}>
              {isRecording ? 'Записът е активен...' : 'Гласов режим (Потребител 1)'}
            </Text>
          </View>
          <View style={styles.divider} />
          <TouchableOpacity
          ...
          >
            <Text style={styles.halfButtonText}>
              {isRecording ? 'Натиснете за КРАЙ и ИЗПРАЩАНЕ' : 'Натиснете за СТАРТ на запис'}
            </Text>
          </TouchableOpacity>
        </View>
      );
    ...
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={[styles.halfLabel, { marginBottom: 20 }]}>Самостоятелен екран (Потребител 1)</Text>
        <TouchableOpacity
          style={[styles.halfButton, { backgroundColor: BTN }]}
          onPress={() => handleConfirmMessage('user1', 'Потребител 1 текстови данни')}
        >
  ...
  if (phase === 'history') {
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={[styles.half, { backgroundColor: BG, paddingTop: NOTCH_OFFSET, paddingHorizontal: 12 }]}>
          <Text style={[styles.halfLabel, { marginBottom: 8 }]}>История на диалога</Text>
          <ScrollView style={styles.historyBox} contentContainerStyle={{ paddingBottom: 8 }}>
        ...
        {/* BOTTOM HALF: actions */}
        <View style={[styles.half, { backgroundColor: BG, paddingTop: 20, paddingHorizontal: 15, justifyContent: 'flex-start' }]}>
          ...
          >
            <Text style={styles.halfButtonText}>Въведи отговор ({turnLabel})</Text>
          </TouchableOpacity>
  ...
  if (phase === 'user2-reply') {
    return (
      <View style={styles.container}>
        {/* TOP HALF */}
        <View style={[styles.half, { backgroundColor: BG, paddingTop: NOTCH_OFFSET, paddingHorizontal: 15 }]}>
          <Text style={styles.halfLabel}>Въвеждане на Текст (Потребител 2)</Text>
          <TextInput
            style={styles.inputField}
        ...
        <View style={[styles.half, { backgroundColor: BG, paddingTop: 20, paddingHorizontal: 15 }]}>
          ...
          <TouchableOpacity style={styles.backLink} onPress={() => setPhase('history')}>
