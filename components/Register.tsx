...
type Phase = 'prompting-channels' | 'registering' | 'haptic-handshake' | 'fallback';
...
function useChannelDetection() {
    ...
    const onHapticRegister = () => {
        setPhase('fallback');
    }


    return { phase, onConfirm, onHapticRegister };
}
...
export default function Register( { 
    ...
    PHASE III (only if user has at least one fast channel (text or voice, otherwise its auto-configured)):
        ...
        3. Every window waits an exact amount for the confirmation with one click, otherwise the answer is that the user doesn't have this disability
        4. In the end the user is prompted whether the picked disabilities are indeed right. If they are -> MainScreen, if they aren't -> repeat the flow
    */


    const { phase, onConfirm, onHapticRegister } = useChannelDetection();

    switch (phase){
        case 'prompting-channels': return <PromptChannel onConfirm={onConfirm}/>;
        case 'fallback': return <BlankScreen />;
        case 'haptic-handshake': return <ConfirmHaptic onRegister={() => onHapticRegister()}/>; // registers users who only accept vibrations
        case 'registering': return <RegisterForm onRegister={onRegister}/>; // -> returns callback when done
    }
