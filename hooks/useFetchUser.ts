...
export function useFetchUser() {
    ...
    // ==========================================
    const [userDisabilities, setUserDisabilities] = useState<Disability[]|null>(null);
    ...
    const isUserBlind =
        profileReady && userDisabilities.includes("blind");
    const isUserDeaf = profileReady && userDisabilities.includes("deaf");
    const isUserMute =
        profileReady && userDisabilities.includes("non-verbal");
    ...
    return { profileReady, isUserBlind, isUserDeaf, isUserMute, isSevereIntersectionWIP };
}